...
import { X, Save, Tags, Trash2, AlertTriangle, RefreshCw, Check } from 'lucide-react';
...
export default function ModalCategoria({ isOpen, onClose, onSave, onDelete, categoriaAEditar }: ModalCategoriaProps) {
    ...
    useEffect(() => {
        if (isOpen) {
            setStep('form');
            if (categoriaAEditar) {
                setNombre(String(categoriaAEditar.nombre || ''));
            } else {
                setNombre('');
            }
        } else {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await onSave({ nombre: String(nombre).trim() });
            setStep('success');
    ...
    return (
        ...
        <style>{`
            ...
            .me-modal { position: relative; background: #fff; border: 3px solid #1a0060; border-radius: 28px; width: 100%; max-width: 480px; box-shadow: 8px 8px 0px #1a0060; overflow: visible; z-index: 2; }
            .me-header { display: flex; align-items: center; justify-content: space-between; padding: 20px 24px; border-bottom: 2.5px solid rgba(26,0,96,0.1); background: rgba(255,225,68,0.6); border-radius: 25px 25px 0 0; }
            .me-header.delete { background: rgba(255,80,80,0.1); }
            ...
            .me-input:focus { border-color: #ffe144; box-shadow: 0 0 0 3px rgba(255,225,68,0.15), 3px 3px 0px #1a0060; background: #fff; }
            ...
            {isOpen && (
                ...
                <div className={`me-header ${step === 'confirm-delete' ? 'delete' : ''}`} onPointerDown={(e) => dragControls.start(e)} style={{ touchAction: 'none' }}>
                <div className="me-header-left">
                <div className="me-header-icon" style={{
                    background: step === 'confirm-delete' ? 'rgba(255,80,80,0.1)' : 'rgba(255,225,68,0.12)',
                        border: `1.5px solid ${step === 'confirm-delete' ? 'rgba(255,80,80,0.25)' : 'rgba(255,225,68,0.3)'}`,
                ...
                {step === 'confirm-delete' ? 'Esta acción no se puede deshacer' : step.startsWith('success') ? 'Operación completada' : esEdicion ? 'Modifica los datos de la categoría' : 'Crea una nueva clasificación de productos'}
                </span>
                </div>
                </div>
                ...
                {step === 'form' && (
                    ...
                    <div>
                    <label className="me-label"><Tags size={13} style={{color:'#ffe144'}}/> Nombre de la categoría</label>
                    <input type="text" className="me-input" placeholder="Ej. Llaveros, Stickers..." value={nombre} onChange={e => setNombre(e.target.value)} required disabled={loading} />
                    </div>
                    ...
                    {loading ? <><span className="animate-spin inline-block"><RefreshCw size={16} /></span> Guardando...</> : <><Save size={16} /> {esEdicion ? 'Actualizar' : 'Guardar'} categoría</>}
                    </motion.button>
                    {esEdicion && onDelete && (
